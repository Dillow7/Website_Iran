...
<head>
    ...
    <style>
        .char-count { font-size:.78rem; color:var(--adm-text-muted); text-align:right; margin-top:4px; }
        .char-count.warn { color:var(--adm-amber); }
        .char-count.over { color:var(--adm-red); font-weight:700; }
        .slug-preview { font-size:.8rem; color:var(--adm-text-muted); margin-top:5px; font-family:monospace; background:var(--adm-bg); padding:4px 8px; border-radius:4px; word-break:break-all; }
        .current-image { display:flex; align-items:center; gap:12px; background:var(--adm-bg); border:1px solid var(--adm-border); border-radius:6px; padding:10px 14px; margin-top:8px; }
        .current-image img { width:64px; height:48px; object-fit:cover; border-radius:4px; flex-shrink:0; }
        .current-image span { font-size:.85rem; color:var(--adm-text-muted); }
    </style>
</head>
<body>
            ...
            <div>
                <div class="adm-breadcrumb">
                    ...
                    <?php if (!empty($article['id'])): ?>
                        <span>›</span>
                        <span style="color:var(--adm-text-mid)">ID <?php echo (int)$article['id']; ?></span>
                    <?php endif; ?>
                </div>
                <h1 class="adm-page-title">Modifier l'article</h1>
                <?php if (!empty($article['created_at'])): ?>
                    <div class="adm-muted" style="margin-top:4px;">
                        Créé le <?php echo htmlspecialchars((string)$article['created_at']); ?>
                    </div>
                <?php endif; ?>
            </div>
            <div style="display:flex;gap:10px;align-items:center;">
                <?php if (!empty($article['category_slug']) && !empty($article['slug'])): ?>
                    <a href="<?php echo SITE_URL; ?>/<?php echo htmlspecialchars($article['category_slug']); ?>/<?php echo htmlspecialchars($article['slug']); ?>"
                       target="_blank" class="adm-btn adm-btn-ghost">
                        ↗ Voir l'article
                    </a>
                <?php endif; ?>
                ...
                <form method="post" enctype="multipart/form-data" id="article-form">
                    ...
                    <div class="adm-row">
                        ...
                        <div>
                            ...
                            <div class="slug-preview" id="slug_preview">
                                <?php echo SITE_URL; ?>/<em id="slug_preview_text"><?php echo field($values, 'slug') ?: 'votre-slug'; ?></em>
                            </div>
                    ...
                    <div class="adm-field">
                        ...
                        <textarea class="adm-textarea" id="meta_description" name="meta_description"
                                  ...
                                  placeholder="Résumé affiché dans les résultats Google"><?php echo field($values, 'meta_description'); ?></textarea>
                    ...
                    <!-- Extrait -->
                    <div class="adm-field">
                        <label class="adm-label" for="excerpt">Extrait <span class="adm-muted">(optionnel)</span></label>
                    ...
                    <div class="adm-row">
                        ...
                        <div>
                            ...
                            <div class="adm-help">Vider le champ pour repasser en brouillon.</div>
                        </div>
                    </div>
                    ...
                    <div class="adm-row">
                        <div>
                            <label class="adm-label" for="image_file">Nouvelle image <span class="adm-muted">(laisser vide pour conserver)</span></label>
                            <input class="adm-input" id="image_file" name="image_file"
                                   type="file" accept="image/*"
                                   style="padding:7px 12px; cursor:pointer;">
                            <?php if (!empty($article['image'])): ?>
                                <div class="current-image">
                                    <img src="<?php echo SITE_URL; ?>/img/<?php echo htmlspecialchars($article['image']); ?>"
                                         alt="Image actuelle" loading="lazy">
                                    <span>Image actuelle : <code><?php echo htmlspecialchars($article['image']); ?></code></span>
                                </div>
                            <?php endif; ?>
                        ...
                        <div>
                            <label class="adm-label" for="alt_image">Texte alternatif (alt)</label>
                    ...
                    <div class="adm-form-actions">
                        <button type="submit" class="adm-btn adm-btn-primary">Enregistrer les modifications</button>
